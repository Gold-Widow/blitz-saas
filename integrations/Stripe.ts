...
// TODO: Once blitz gets server.ts and initHooks typescript support, add a hook to get the stripe secret key there
// and if it fails, we won't start the app
const getStripeSecretKey = (): string => {
  const secretKey = process.env.STRIPE_SECRET_KEY
  if (!secretKey) {
    throw new Error("Stripe secret key not found in variable 'process.env.STRIPE_SECRET_KEY'")
  }
  return secretKey
}
